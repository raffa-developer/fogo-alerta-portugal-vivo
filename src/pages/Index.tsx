...
const Index = () => {
  const { data: fireData, isLoading: isLoadingFireData, isError: isFireError } = useFireData();
  ...
  useEffect(() => {
    if (fireData && !isLoadingFireData) {
      if (fireData.incidents.length > 0) {
        toast.info(`${fireData.incidents.length} incêndios ativos em Portugal`, {
          icon: <Flame className="h-4 w-4 text-fire" />,
        });
      } else {
        toast.success("Não existem incêndios ativos em Portugal neste momento", {
  ...
  useEffect(() => {
    if (isFireError) {
      toast.error("Erro ao carregar dados de incêndios. Tente novamente mais tarde.");
    }
  }, [isFireError]);

  // Get top risk districts (for highlighting)
  const getHighRiskDistricts = () => {
    if (!fireData || !fireData.riskLevels) return [];
    ...
    <div className="min-h-screen bg-gray-50 flex flex-col">
      ...
      <main className="flex-1">
        <div className="container px-4 py-6 md:px-6 md:py-8">
          <h2 className="text-2xl font-bold mb-6">Monitorização de Incêndios em Portugal</h2>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4 lg:gap-6">
            ...
            <div className="space-y-4">
              ...
              <div className="space-y-3">
                <h3 className="font-semibold text-lg">Distritos de Alto Risco</h3>
          ...
          <div className="mt-8 text-center text-sm text-muted-foreground">
            <p>Dados atualizados em: {fireData ? new Date(fireData.timestamp).toLocaleString('pt-PT') : 'Carregando...'}</p>
            <p className="mt-1">Fonte: Dados obtidos através de APIs públicas da Autoridade Nacional de Emergência e Proteção Civil</p>
          </div>
        </div>
      </main>
    </div>
